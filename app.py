...
if os.path.exists(GOOGLE_CREDS_PATH):
    ...
    logger.info("Please place the credentials file in the project root directory")
...
def status():
    ...
    return jsonify({
        ...
        "monitoring_active": monitoring_thread is not None and monitoring_thread.is_alive(),
        "sheet_id": os.getenv('SHEET_ID'),
        "google_creds_configured": os.path.exists(GOOGLE_CREDS_PATH),
        "timestamp": datetime.now().isoformat()
    })
...
def setup_chrome_driver():
    ...
    try:
        ...
        if RENDER_ENVIRONMENT:
            ...
            chromedriver_path = os.getenv('CHROMEDRIVER_PATH', "/home/user/chromedriver")
            ...
            # Additional Render-specific arguments
            chrome_options.add_argument('--disable-software-rasterizer')
            chrome_options.add_argument('--disable-dev-tools')
            chrome_options.add_argument('--no-zygote')
            chrome_options.add_argument('--single-process')
            
            # Use ChromeDriver from user directory
            driver = uc.Chrome(
                options=chrome_options,
                driver_executable_path=chromedriver_path,
                version_main=137,  # Updated to match current Chrome version
                use_subprocess=False  # Avoid subprocess issues in container
            )
        else:
...
if __name__ == '__main__':
    try:
        ...
        # Get port from environment variable with explicit type conversion and error handling
        try:
            port = int(os.environ.get('PORT', 10000))
        except ValueError:
            logger.error("Invalid PORT environment variable value. Using default port 10000.")
            port = 10000
            
        # Add host binding logging
        logger.info(f"Starting Flask app on host='0.0.0.0' port={port}")
        
        # Run the Flask app with production configuration
        app.run(
            host='0.0.0.0',
            port=port,
            debug=False,
